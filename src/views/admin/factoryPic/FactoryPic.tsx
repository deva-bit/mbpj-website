...
const FactoryPic = () => {
  ...
  const [isDataFactory, setFactory] = useState<FactoryResponseDto[]>([]);
  ...
  const [deleteValue, setDeleteValue] = useState<factoryPicDeleteRequestDto>({
    id: 0,
  })
  const {
  ...
  return (
    <div>
      <Spin spinning={isLoadingFactoryPIC} tip="Loading...">
        <Card title="Factory PIC" extra={<div className="button-container">
          <Button type="primary" onClick={() => setIsModalOpen(true)}><PlusOutlined rev={undefined} /> Add Factory PIC</Button>{isModalOpen && <AddModalFactoryPic successfullAdd={successfullAdd} factory={isDataFactory} loadingFactoryCode={isLoadingFactory} closeModal={() => setIsModalOpen(false)} />}</div>} className="my-card">
